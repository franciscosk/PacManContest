        ...
        while node != goal:
            ...
            for successor, edge in successors:
                ...
                if residual > 0 and not intPath in path and intPath not in visited:
                    visited.add(intPath)
                    frontier.push(
                        (successor, path + [(edge, residual)], pathCost + 1))
...
class BaseAgent(CaptureAgent):
    ...
    def observe(self, enemy, gameState):
        ...
        teamPos = [gameState.getAgentPosition(
            team) for team in self.getTeam(gameState)]
        allObs = util.Counter()

        for pos in self.legalPositions:
            teamDist = [
                team for team in teamPos if util.manhattanDistance(team, pos) <= 5]
            if teamDist:
                allObs[pos] = 0.0
            else:
                trueDistance = util.manhattanDistance(myPos, pos)
                posProb = gameState.getDistanceProb(
                    trueDistance, noisyDistance)
    ...
    def getAnticipatedGhosts(self, gameState):
        ...
        for enemy in self.getOpponents(gameState):
            ...
            enemyGameState = gameState.getAgentState(
                enemy) if anticipatedPos else None
    ...
    def findBottlenecks(self, gameState, defMode=True):
        if defMode:
            ...
            endingPos = self.getFoodYouAreDefending(
                gameState).asList() + self.getCapsulesYouAreDefending(gameState)
            network, source = self.getFlowNetwork(
                gameState, startingPos=startingPos)
        else:
            startingPos = self.enemyBoundaryPos
            endingPos = self.getFood(
                gameState).asList() + self.getCapsules(gameState)
            network, source = self.getFlowNetwork(
                gameState, startingPos=startingPos, defMode=defMode)
    ...
    def getFlowNetwork(self, gameState, startingPos=None, endingPos=None, defMode=True):
        ...
        if self.red:
            def atHome(x): return (x < walls.width /
                                   2) if defMode else lambda x: (x >= walls.width / 2)
        else:
            def atHome(x): return (x >= walls.width /
                                   2) if defMode else lambda x: (x < walls.width / 2)

        def actionPos(x, y): return [(x + 1, y),
                                     (x - 1, y), (x, y + 1), (x, y - 1)]
        possiblePos = [(x, y) for x, y in legalPositions if (
            x, y) in legalPositions and (atHome(x))]
    ...
    def getTopkBottleneck(self, gameState, k, defMode=True):
        bottleneckCounter, bottleneckPos = self.findBottlenecks(
            gameState, defMode=defMode)
    ...
    def getDepthPos(self, gameState, bottleNeckPos, offBottleNeckCount):
        ...
        for food in foodsRemaining:
            foodDepth[food] = 0
        foodDepth = {k: v for k, v in sorted(
            foodDepth.items(), key=lambda item: item[1])}
        return foodDepth
    ...
    def getMinGhostAnticipatedDistance(self, pacmanPos):
        global ANTICIPATER
        dists = [(ghostState, self.getMazeDistance(pacmanPos, ghostPos))
                 for ghostState, ghostPos in ANTICIPATER]
        minGhostState, minDis = min(
            dists, key=lambda t: t[1]) if dists else (None, 999999)
    ...
    def allInvadersKilled(self, myCurrPos, gameState, nextAction):
        ...
        if len(invaders) > 0:
            dists = [self.getMazeDistance(
                myCurrPos, a.getPosition()) for a in invaders]
        ...
        if currentInvaderDistance == 1:
            ...
            nextEnemies = [successor.getAgentState(
                i) for i in self.getOpponents(successor)]
            nextInvaders = [
                a for a in nextEnemies if a.isPacman and a.getPosition() != None]
    ...
    def getBoundaryPos(self, gameState, span=4, defMode=True):
        ...
        xSpan = [
            x - i for i in range(span)] if flag else [x + i for i in range(span)]
        walls = gameState.getWalls().asList()
        homeBound = list()
        for x in xSpan:
            pos = [(int(x), y) for y in range(layout.height) if (
                x, y) not in walls and (x + enemy, y) not in walls]
    ...
    def checkIfStuck(self, myCurrPos):
        ...
        if len(self.history.list) < 8:
            ...
            try:
                ...
                if PDDL_DEBUG:
                    print("!! STUCK PROBLEM - Reinit History !!")
                if PDDL_DEBUG:
    ...
    def updatePoweredStatus(self, agentPos, gameState, prevgameState):
        ...
        capsules = self.getCapsules(
            prevgameState) if prevgameState else self.getCapsules(gameState)
        if agentPos in capsules:
            if PDDL_DEBUG:
                print("PacMan Powered - No depth limit now")
            POWERED_TIMER = 40 * 2
            POWERED = True
            DEPTH_LIMIT = -1
            ...
            if POWERED_TIMER > 0:
                ghostState, currMinDis = self.getMinGhostAnticipatedDistance(
                    agentPos)
                ...
                if spawnedGhostPos:
                    ...
                    if PDDL_DEBUG:
                        print(
                            f'Ghost Spawned: {spawnedGhostPos}, {distance} - Powered Down')
                    POWERED = False
                    POWERED_TIMER = 0
                else:
                    POWERED = True
...
class MasterAgent(BaseAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        self.obs = {}
        for enemy in self.getOpponents(gameState):
            self.enemyStartPositions.append(
        ...
        self.enemyBoundaryPos = self.getBoundaryPos(
            gameState, span=1, defMode=False)
        self.masterCapsules = self.getCapsulesYouAreDefending(gameState)
        self.offensiveCapsules = self.getCapsules(gameState)
        self.isNoCapsuleMap = True if len(
            self.offensiveCapsules) == 0 else False
        self.masterFoods = self.getFoodYouAreDefending(gameState).asList()
        tic = time.perf_counter()
        self.bottleNeckCount, self.bottleNeckPos = self.getTopkBottleneck(
            gameState, 5)
        self.offBottleNeckCount, self.offBottleNeckPos = self.convertBottleNeckOff(gameState, self.bottleNeckCount,
                                                                                   self.bottleNeckPos)
        self.offBottleNecksDepth = self.getDepthPos(
            gameState, self.offBottleNeckPos, self.offBottleNeckCount)
        if FOOD_DEPTHS is None:
            FOOD_DEPTHS = self.offBottleNecksDepth
        self.depthFoodCounts = len(
            [d for d in FOOD_DEPTHS.values() if d <= DEPTH_LIMIT])
        toc = time.perf_counter()
        if PDDL_DEBUG:
            print(f"Ran in {toc - tic:0.4f} seconds")

        # diverging strategy
        self.initialDivergingStrategy = True if len(
            self.offensiveCapsules) == 0 else False
    ...
    def chooseAction(self, gameState):
        ...
        if PDDL_DEBUG:
            print(f"======MODE: #{self.index} - {self.AGENT_MODE}======")
        start = time.time()
        try:
            with time_limit(self.TIME_LIMIT):
                ...
                self.isNoCapsuleMap = True if len(
                    self.getCapsules(gameState)) == 0 else False
                ...
                if self.index == 0 or self.index == 1 or len(ANTICIPATER) == 0:
                    ...
                    if PDDL_DEBUG:
                        print(
                            f"Ghosts Anticipated: {[ghostPos for ghost, ghostPos in ANTICIPATER]}")

                # Update Agent Powered Status
                self.updatePoweredStatus(
                    myCurrPos, gameState, self.getPreviousObservation())
                ...
                if len(self.masterCapsules) == 0:
                    if PDDL_DEBUG:
                        print("No capsules - no depth limit")
                    DEPTH_LIMIT = -1
                    DEPTH_LIMIT_ON = False
                ...
                if (self.index == 2 or self.index == 3) \
                    ...
                    returnedAction = self.checkAndGetBlockingDefensiveAgent(
                        gameState)
                ...
                if self.AGENT_MODE == "BLOCKING" and (self.index == 2 or self.index == 3):
                    returnedAction = self.handleBlockingAgentFlow(
                        myCurrPos, gameState)
                    if returnedAction is not None:
                        return returnedAction
                ...
                if self.isNoCapsuleMap and self.AGENT_MODE != "APPROXIMATE":
                    if MCTS_DEBUG:
                        print("No Capsule Map Now - Try MCTS")
                    returnedAction = self.getMCTSAgent().chooseAction(gameState.deepCopy())
                    if MCTS_DEBUG:
                        print(f"MCTS Action: {returnedAction}")
                    if returnedAction is not None:
                        if MCTS_DEBUG:
                            print('Eval time for agent %d: %.4f seconds' %
                ...
                if not self.approximationMode:
                    ...
                    if self.AGENT_MODE == "ATTACK" and (self.index == 0 or self.index == 1) and not scared:
                        returnedAction = self.agent1AttackingFlow(
                            myCurrPos, foods, gameState)
                        if returnedAction is not None:
                            return returnedAction
                ...
                if self.AGENT_MODE == "ATTACK" \
                    ...
                    returnedAction = self.agent2AttackingFlow(
                        myCurrPos, foods, gameState)
                    if returnedAction is not None:
                        return returnedAction
            ...
            if PDDL_DEBUG:
                print('==== Time Limit Exceeded: #%d: %.4f seconds ====' %
                      (self.index, time.time() - start))
    ...
    def checkAndGetBlockingDefensiveAgent(self, gameState):
        ...
        cFood = len(self.masterFoods) - len(list(set(self.masterFoods) -
                                                 set(self.bottleNeckPos[bestBottleNeckPos])))
        cCapsule = len(
            [cap for cap in self.masterCapsules if cap in self.bottleNeckPos[bestBottleNeckPos]])
        ...
        if cCapsule == len(self.masterCapsules):
            ...
            if (len(self.masterCapsules) == 0
                ...
                if PDDL_DEBUG:
                    print(
                        f"Turning defensive agent into blocking agent - {bottleNeckFood * 100}% covered")
    ...
    def handleBlockingAgentFlow(self, myCurrPos, gameState):
        ...
        if myCurrPos == self.start:
            ...
            if PDDL_DEBUG:
                print("Died being blocker - switch back")
            self.agent = self.getDefendModeAgent(gameState)
            DEFENSIVE_BLOCKING_MODE = False
        else:
            bestBottleNeckPos, bestBottleNeckCount = self.bottleNeckCount[0]
    ...
    def makeApproximationAgent(self):
        ...
        if PDDL_DEBUG:
            print(f"Approximate Move: {self.movesApproximating}")
        if self.AGENT_MODE != "APPROXIMATE" and self.AGENT_MODE == "ATTACK":
            # initilize ApproximateQLearning
            if PDDL_DEBUG:
                print("Turning Attack PDDL into Approximating Agent")
            if self.index == 0 or self.index == 1:
    ...
    def handleApproximationFlow(self, myCurrPos, gameState):
        ...
        if ((self.index == 0 or self.index == 1)
            ...
            if PDDL_DEBUG:
                print("Food Eaten/Threshold Reached -> switching back to Attack PDDL")
            self.history = Queue()
            self.approximationMode = False
            self.movesApproximating = 0
            ...
            self.movesApproximating = 0
            if PDDL_DEBUG:
                print("back home - attack PDDL ON - stop approximating")
            self.agent = self.getAttackModeAgent(gameState)
    ...
    def atBoundaryLogic(self, myCurrPos, gameState):
        ...
        if PDDL_DEBUG:
            print("At Boundary - Food Collected/Resetting")
        newScore = self.getScore(gameState)
        if self.index == 0 or self.index == 1:
        ...
        if TOTAL_FOOD_COLLECTED >= self.depthFoodCounts - 3:
            if PDDL_DEBUG:
                print("[FailSafe] Turning off depth limit")
            DEPTH_LIMIT_ON = False
            DEPTH_LIMIT = -1

    def setAgent1ClosestFood(self, myCurrPos, foods):
        global AGENT_1_CLOSEST_FOOD
        foodDists = [(food, self.getMazeDistance(myCurrPos, food))
                     for food in foods]
        if len(foodDists) > 0:
    ...
    def cleanAgent(self, gameState):
        ...
        if self.AGENT_MODE == "APPROXIMATE":
            ...
            self.movesApproximating = 0
            if PDDL_DEBUG:
                print("Turning Approximate agent into attacking agent")
            self.agent = self.getAttackModeAgent(gameState)
...
class OffensivePDDLAgent(BaseAgent):
    ...
    def generatePDDLFluentStatic(self, gameState, remove=[]):
        ...
        for pos in allPos:
            if (pos[0] + 1, pos[1]) in allPos:
                connected.append(
                    f'\t\t(connected cell{pos[0]}_{pos[1]} cell{pos[0] + 1}_{pos[1]})\n')
            if (pos[0] - 1, pos[1]) in allPos:
                connected.append(
                    f'\t\t(connected cell{pos[0]}_{pos[1]} cell{pos[0] - 1}_{pos[1]})\n')
            if (pos[0], pos[1] + 1) in allPos:
                connected.append(
                    f'\t\t(connected cell{pos[0]}_{pos[1]} cell{pos[0]}_{pos[1] + 1})\n')
            if (pos[0], pos[1] - 1) in allPos:
                connected.append(
                    f'\t\t(connected cell{pos[0]}_{pos[1]} cell{pos[0]}_{pos[1] - 1})\n')
        ...
        if len(ANTICIPATER) == 0:
            ...
            for ghost in ghosts:
                ...
                if ghost.scaredTimer <= 3:
                    has_ghost.append(
                        f'\t\t(has-ghost cell{int(ghostPos[0])}_{int(ghostPos[1])})\n')
        else:
            if PDDL_DEBUG:
                print("Adding anticipater logic")
            for ghostState, ghostPos in ANTICIPATER:
                if ghostPos:
                    ...
                    if foodDepth <= DEPTH_LIMIT:
                        if AGENT_1_CLOSEST_FOOD and self.index == 2 or self.index == 3:
                            if foodPos != AGENT_1_CLOSEST_FOOD:
                                at_food.append(
                                    f'\t\t(at-food food{i + 1} cell{foodPos[0]}_{foodPos[1]})\n')
                        else:
                            at_food.append(
                                f'\t\t(at-food food{i + 1} cell{foodPos[0]}_{foodPos[1]})\n')
        else:
    ...
    def getFoodFluent(self, index, foodPos, agentPos, gameState):
        ...
        if foodPos in FOOD_DEPTHS:
            ...
            # failsafe
            foodDepth = 0
        ghostState, minGhostDistance = self.getMinGhostAnticipatedDistance(
            agentPos)
        allowFoodAdd = True
    ...
    def generatePddlGoal(self, gameState, features):
        ...
        scaredTimer = 99999
        if len(ghosts) > 0:
            ghostDistance, scaredTimer = self.getGhostDistanceAndTimers(
                pacmanPos, ghosts, ghostsState)
            thres = features["threshold"]
        ...
        if features["problemObjective"] is None:
            closestHome, closestCap = self.compareCapsuleAndHomeDist(
                gameState, pacmanPos)
            # gameTimeLeft decrease by 4 for every 1 move - anticipate + come back
            if ((closestHome * 4) + 100) >= gameTimeLeft:
                if PDDL_DEBUG:
                    print(f"Timer Objective #1: {closestHome}")
                problemObjective = self.goBackHomeHardObjective(
                    gameState, goal, pacmanPos)
            ...
            problemObjective = self.goToBoundaryPoint(
                goal, features, gameState, pacmanPos)
        else:
            # fallback goals
            problemObjective = self.tryFallBackGoals(
                goal, features, gameState, pacmanPos)
    ...
    def getGhostDistanceAndTimers(self, pacmanPos, ghosts, ghostsState):
        ...
        if PDDL_DEBUG:
            print(
                f'Ghost Alert with Dist: {ghostDistance} | scaredTimer: {scaredTimer}')
    ...
    def compareCapsuleAndHomeDist(self, gameState, pacmanPos):
        ...
        if len(self.getCapsules(gameState)) > 0:
            closestCap = min([self.getMazeDistance(pacmanPos, cap)
                             for cap in self.getCapsules(gameState)])
            closestHome = min([self.getMazeDistance(pacmanPos, pos)
                              for pos in self.homePos if pos[0] == x])
        else:
            closestHome = 1
            closestCap = 10
    ...
    def getFlag(self, gameState, threshold, foods, pacmanPos):
        ...
        if POWERED:
            if PDDL_DEBUG:
                print("Agent powered - no threshold")
            return False
        ...
        if self.currScore < 0 and totalFoodEaten + self.currScore > 0 \
            ...
            if PDDL_DEBUG:
                print("Defensive Blocking: BACK HOME")
            return True
        try:
            foodEatenPer = totalFoodEaten / TOTAL_FOODS
        except:
            if PDDL_DEBUG:
                print("Re-syncing total foods")
            TOTAL_FOODS = len(self.getFood(gameState).asList())
            foodEatenPer = 0
        if PDDL_DEBUG:
        ...
        if len(foodDists) > 0:
            minDistance = min([self.getMazeDistance(
                pacmanPos, food) for food in foods])
        else:
            minDistance = 99999
        # so close to food - eat and then run back
        if self.areWeWinning(gameState):
            if PDDL_DEBUG:
                print(f"Game Winning - Be bit greedy")
            flag = True if (
                foodEatenPer > threshold and minDistance > 1) else False
        else:
            if PDDL_DEBUG:
                print(
                    f"Game is at Tie/Not Winning - Hard Threshold: {threshold * 100}%")
    ...
    def goBackHomeHardObjective(self, gameState, goal, pacmanPos):
        ...
        if pacmanPos in self.homePos:
            goal.append(
                f'\t\t(at-pacman cell{self.start[0]}_{self.start[1]})\n')
        else:
            goal.append('\t\t(or\n')
    ...
    def goToBoundaryPoint(self, goal, features, gameState, pacmanPos):
        if PDDL_DEBUG:
            print('Objective goToBoundaryPoint')
        boundaryPoint = features["reachBoundaryPoint"]
        goal.append(
            f'\t\t(at-pacman cell{boundaryPoint[0]}_{boundaryPoint[1]})\n')
    ...
    def generatePddlProblem(self, gameState, features):
        ...
        goalStatement, goalObjective = self.generatePddlGoal(
            gameState, features)
    ...
    def chooseAction(self, gameState, overridefeatures=None):
        ...
        if overridefeatures:
            ...
            if "reachBoundaryPoint" in overridefeatures:
                if PDDL_DEBUG:
                    print("Overriding reachBoundaryPoint")
        ...
        if DEFENSIVE_BLOCKING_MODE:
            if PDDL_DEBUG:
                print("Defensive blocking - Threshold down to 50%")
        ...
        if len(myFoods) > 0:
            distToFood = min([self.getMazeDistance(
                agentPosition, food) for food in myFoods])
        else:
            distToFood = 99999
    ...
    def tryFallbackPlans(self, gameState, features, problemObjective):
        ...
        if problemObjective == "EAT_CAPSULE":
            ...
            if PDDL_DEBUG:
                print("No plan found for GO_TO_BOUNDARY_POINT - Normal Flow")
            features["problemObjective"] = None
            return self.getPlan(gameState, features)
    ...
    def getPlan(self, gameState, features):
        problem_file, problemObjective = self.generatePddlProblem(
            gameState, features)
    ...
    def checkBlindSpot(self, agentPosition, gameState, features):
        ...
        if len(ghostsPos) > 0:
            ghostsDist = [(ghost, self.getMazeDistance(
                agentPosition, ghost)) for ghost in ghostsPos]
            minGhostPos, minGhostDistance = min(ghostsDist, key=lambda t: t[1])
            minGhostEucDistance = self.euclideanDistance(
                agentPosition, minGhostPos)
...
class ApproxQLearningOffense(BaseAgent):
    ...
    def chooseAction(self, gameState):
        ...
        global AGENT_1_FOOD_EATEN, AGENT_2_FOOD_EATEN
        if PDDL_DEBUG:
            print(f'======New Offensive Approx Action: #{self.index}========')
        legalActions = gameState.getLegalActions(self.index)
        agentPosition = gameState.getAgentPosition(self.index)

        if PDDL_DEBUG:
            print(f'Pacman at {agentPosition}')
...
class FeaturesExtractor:
    ...
    def getFeatures(self, gameState, action):
        ...
        walls = gameState.getWalls()
        enemies = [gameState.getAgentState(
            i) for i in self.agentInstance.getOpponents(gameState)]
        ghosts = [a.getPosition()
                  for a in enemies if not a.isPacman and a.getPosition() != None]
        ...
        if features is None:
            if len(ANTICIPATER) == 0:
                ...
                for i, invader in enumerate(invaders):
                    ...
                    has_invaders.append(
                        f'\t\t(at-invader invader{i + 1} cell{int(invaderPos[0])}_{int(invaderPos[1])})\n')
            else:
                if PDDL_DEBUG:
                    print("Adding anticipater invader logic")
            ...
            has_invaders.append(
                f'\t\t(at-invader invader0 cell{int(blockingPos[0])}_{int(blockingPos[1])})\n')
        ...
        if features is None:
            if len(ANTICIPATER) == 0:
                ...
                for i, invader in enumerate(invaders):
                    invaderPos = invader.getPosition()
                    goal.append(
                        f'\t\t(not (at-invader invader{i + 1} cell{int(invaderPos[0])}_{int(invaderPos[1])}))\n')
            else:
                if PDDL_DEBUG:
                    print("Adding anticipater invader logic")
                for i, invaderTup in enumerate(ANTICIPATER):
            ...
            blockingPos = features["BlockingPos"]
            goal.append(
                f'\t\t(not (at-invader invader0 cell{int(blockingPos[0])}_{int(blockingPos[1])}))\n')
            invaders = []
        ...
        if not invaders:
            ...
            if not Eaten:
                if myPos not in self.boundaryPos and len(enemyHere) == 0:
                    if PDDL_DEBUG:
                        print(f'Going to #1')
                    goal.extend(self.generateRedundantGoal(
                        self.boundaryPos, myPos))
...
def nearByEnemies(our_agent_index: int, gameState: GameState):
    ...
    enemy_states = [(gameState.getAgentState(enemy_index), enemy_index)
...
def simulateEnemyPacman(our_agent_index: int, gameState: GameState, distancer: Distancer,
    ...
    for enemy_pos, enemy_index in close_enemy_pacmans:
        ...
        enemy_actions = gameState.getLegalActions(
            enemy_index)  # this includes stop
        ...
        for enemy_action in enemy_actions:
            ...
            if distanceToAgent > 4:
                ...
                new_distances = [distancer.getDistance(
                    next_enemy_pos, escape_pos) for escape_pos in enemy_escape_positions]
                min_escape_dist[enemy_action] = min(new_distances)
            else:
                min_escape_dist[enemy_action] = -1 * \
                    distancer.getDistance(next_enemy_pos, my_pos)
        ...
        if len(best_actions) > 1:
            ...
            for action in best_actions:
                ...
                action_dist[action] = min([distancer.getDistance(
                    pos, escape_pos) for escape_pos in enemy_escape_positions])
...
def simulatedEnemyPacmanConsiderBoth(enemy_agent_index: int, gameState: GameState, distancer: Distancer,
    ...
    dangerous_our_team = dangerousEnemies(
        enemy_agent_index, gameState)  # our agents
    ...
    if dangerous_our_team != []:
        distances = [(distancer.getDistance(enemy_pos, pos), pos)
                     for pos, index in dangerous_our_team]
...
class MctsAgent(BaseAgent):
    ...
    def chooseAction(self, gameState: GameState):
        ...
        try:
            ...
            our_escape_positions = self.getBoundaryPos(
                gameState, span=1, defMode=True)
            my_pos = gameState.getAgentPosition(self.index)
            min_escape_distance = min([self.distancer.getDistance(
                my_pos, escape_pos) for escape_pos in our_escape_positions])
            ...
            if MCTS_DEBUG:
                print(
                    f"index {self.index}, depth {mcts.mctsIntialNode.num_times_visited}")
            ...
            expected_actions = getBestPathFromNodeActionPosition(
                self.expected_next_tree, self.index)
            self_shared_info = getattr(MctsAgent, f"agent{self.index}")
            self_shared_info["expected_actions"] = expected_actions

            if MCTS_DEBUG:
                print(
                    f"index {self.index}, action {action}, depth {mcts.mctsIntialNode.num_times_visited}")
            return action
        except TimeoutException as e:
            raise
        except Exception as e:
...
class MctsProblem(MDPproblem):
    ...
    def generateSuccessor(self, state, action, numberOfAgentTurns=None):
        ...
        # the first enemy is the one that will move directly after our turn
        first_enemy_index = (our_agent_index + 1) % 4
        second_enemy_index = (our_agent_index + 3) % 4
        original_enemies_state = [
            gameState.getAgentState(i) for i in enemies_index]
        ...
        original_enemy_pos = [enemy.getPosition()
                              for enemy in original_enemies_state]
        ...
        if numberOfAgentTurns is not None:
            ...
            team_index = getTeamMateIndexes(
                our_agent_index, our_agent_red, gameState)[0]
            ...
            if numberOfAgentTurns <= len(expected_action) - 1:
                ...
                team_mate_action, team_position_action_applied_from = expected_action[
                    numberOfAgentTurns]
        ...
        ########## first_enemy_index############
        enemy_escape_positions = self.agent.getBoundaryPos(
            succ_state, span=1, defMode=False)
        ...
        if team_mate_action:
            ...
            if succ_state.getAgentPosition(team_index) == team_position_action_applied_from:
                ...
                if self.agent.distancer.getDistance(team_position_action_applied_from, original_my_pos) < 7:
                    succ_state = succ_state.generateSuccessor(
                        team_index, team_mate_action)
        ...
        # record metrics
        new_enemies_state = [
            succ_state.getAgentState(i) for i in enemies_index]
        ...
        for old_pos, new_pos in zip(original_enemy_pos, new_enemy_pos):
            if old_pos is None or new_pos is None:
                if MCTS_DEBUG:
                    print("ERROR: a position had a None value")
        ...
        new_state["score_change"] = new_score - \
            original_score - team_mate_score_change
        ...
        for index, eaten in zip(enemies_index, eaten_enemies):
            if eaten:
                ...
                enemy_agent_eaten_food_dropped += gameState.getAgentState(
                    index).numCarrying
    ...
    def _eq_gameState_helper(self, gameState1, gameState2, consider_team):
        ...
        if not gameState_data1.food == gameState_data2.food:
            return False
        if not gameState_data1.capsules == gameState_data2.capsules:
            return False
        if not gameState_data1.score == gameState_data2.score:
            return False
        ...
        agentStates = list(zip(gameState_data1.agentStates,
                           gameState_data2.agentStates))
        ...
        for agentState1, agentState2 in agentStates:
            if not self._eq_agentState_helper(agentState1, agentState2):
                return False

        return True

    def _eq_agentState_helper(self, agentState1, agentState2):
        if not agentState1.scaredTimer == agentState2.scaredTimer:
            return False
        if not self._eq_configuration_helper(agentState1.configuration, agentState2.configuration):
            return False
        return True
...
class MctsNode:
    ...
    def createChild(self, action, state, child_available_actions):
        ...
        '''
        child_node = MctsNode(state, parentNode=self, action=action,
...
def pprint_tree(node, agent_index, file=None, _prefix="", _last=True):
    agent_pos = node.state['gameState'].getAgentState(
...
class Mcts():
    ...
    def __init__(self, problem: MctsProblem, tree_policy, simulation_policy, learning_rate=1,
        ...
        if existing_tree is not None and problem.compareStatesEq(startState, existing_tree.state, consider_team=True):
            ...
            self.mctsIntialNode = self.initalise_tree(
                startState, available_action)
    ...
    def iterate(self, time_limit_seconds: float, number_of_itr: int):
        ...
        while (datetime.now() - start_time < time_limit) and itr <= number_of_itr:
            ...
            node_to_simulate = self.tree_policy.select_node_prune(
                root_node=self.mctsIntialNode)

            simulation_reward = self.simulation_policy.estimate_value(
                node=node_to_simulate)
    ...
    def backPropergate(self, simulated_node: MctsNode):
        ...
        while not (curr_node.is_root()):  # this condition means we have reached the parent
            ...
            reward = self.problem.getReward(
                parent_node.state, best_action, best_child_state)
            parent_node.v = (1 - self.learning_rate) * parent_node.v \
                + self.learning_rate * \
                (reward + self.problem.discount * best_child_reward)
        ...
        while (True):
            ...
            while self.fully_expanded(node):
                ...
                if not self.has_living_children(node):
                    ...
                    if node.is_root():  # if this is the root node we have a problem
                        # This required error handling be caller
            ...
            while not self.fully_expanded(node):
                ...
                for action_choosen in possible_actions:
                    ...
                    child_state = self.problem.generateSuccessor(
                        node.state, action_choosen, numberOfAgentTurns=depth)
                    child_available_actions = self.problem.getPossibleActions(
                        child_state)
                    node.createChild(action_choosen, child_state,
                                     child_available_actions)
                ...
                if simulation_candidate_nodes:
                    ...
                    child_to_simulate = random.choice(
                        simulation_candidate_nodes)
...
class UctTreePolicy(TreePolicy):
    ...
    def choose_child(self, node):
        ...
        for childNode in node.childrenNodes.values():
            ...
            childValue = childNode.v + \
                (Cp * math.sqrt((2 * math.log(node.num_times_visited)) /
                 num_time_child_visited))
...
class MonteCarloRandomSimulation(SimulationPolicy):
    ...
    def estimate_value(self, node: MctsNode):
        ...
        if self.depth == 0:
            ...
            cumulative_reward += self.problem.getRewardQuick(
                from_quick_state, None, None)
        else:
            for depth in range(self.depth):
                actions = self.problem.getPossibleActionsQuick(
                    from_quick_state)
